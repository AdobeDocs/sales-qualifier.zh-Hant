---
title: Sales Qualifier概觀
description: 瞭解Sales Qualifier，這是人工智慧驅動的應用程式，可為B2B銷售團隊自動化潛在客戶資格、外聯和買家參與。
feature: Agentic AI, Sales Insights, Account Journeys
role: User
TQID: 'https://experienceleague.adobe.com/4IsAlgFBxddHCz0-CLCDXPCHI7m3motiXhgCR6MZq0k'
product_v2: id: d0a3eab4-7b10-4d96-a71e-6c0f8e7b7c87
feature_v2: id: fc7979f3-56c3-43ca-9784-f1ea3dc69c4b
topic_v2: id: aa2f3246-cb95-4b30-8899-fdf7d73550ccid: bce87dde-a4ab-44c9-8a18-ad66e4ddb377id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: 08dd05e1d13b501d43d457e6217a43aaabdb1d0d
workflow-type: tm+mt
source-wordcount: 412
ht-degree: 22%

---


# 銷售限定詞

Adobe Sales Qualifier是AI支援的應用程式，可自動化企業開發代表(BDR)的潛在客戶資格、拓展和跨管道購買者參與。 Account Qualification Agent會分析您的帳戶和潛在客戶、將已準備好進行下一步的帳戶和潛在客戶排定優先順序，並根據您的CRM資料草擬個人化外展方案。

BDR可使用瀏覽器和電子郵件外掛程式，直接在CRM或Outlook中存取商業智慧。

## 在Sales Qualifier中可以做什麼

* **使用AI產生的活動摘要和訊號式優先順序，優先處理潛在客戶和帳戶**。
* **建立目標導向的參與計畫**，其中AI會為每個潛在客戶建議步調並草擬個人化電子郵件。
* **在一個佇列中處理外展工作**，處理電話、LinkedIn InMails和電子郵件評論。
* **連線行事曆並將個人預約連結插入您的步調，以自動預約會議**。
* **在知識中心建立行動手冊，以您自己的資料通知AI外展人員**。
* **使用AI Chat （使用您的CRM、參與和知識中心資料）以自然語言提出問題**。
* 使用電子郵件和會議預約報告&#x200B;**追蹤外聯績效**。

>[!VIDEO](https://video.tv.adobe.com/v/3476550)

## 探索指南

::::landing-cards-container
:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/circle-play.svg)

開始使用

完成使用者群組和CRM連線的一次性管理員設定。

[開始使用](getting-started.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/list-check.svg)

潛在客戶

建立、篩選及檢閱您的潛在客戶清單。

[潛在客戶](prospects.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/list-check.svg)

帳戶

使用帳戶層級的管道和參與資料排定外聯活動的優先順序。

[帳戶](accounts.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/code-branch.svg)

傳出工作流程

使用AI產生的個人化電子郵件建立目標導向步調。

[傳出工作流程](outbound-workflows.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/list-check.svg)

工作

在一個佇列中處理電話通話、LinkedIn InMail和電子郵件稽核工作。

[工作](tasks.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/list-check.svg)

績效

檢閱電子郵件和會議預約績效。

[傳出效能](performance.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/list-check.svg)

知識中心

建立行動手冊，在您的資料中納入人工智慧外聯和協助。

[知識中心](knowledge-center.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/circle-play.svg)

整合

連線Salesforce或Microsoft Dynamics 365並對應您的欄位。

[整合](integrations.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/list-check.svg)

輪廓設定

設定您的電子郵件、簽名和行事曆可用性。

[輪廓設定](profile-settings.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/list-check.svg)

使用者角色和許可權

瞭解使用者和管理員群組模型。

[使用者角色和許可權](user-roles-permissions.md)
:::

:::card
![圖示](https://cdn.experienceleague.adobe.com/icons/circle-play.svg)

AI聊天

根據您的CRM、參與和知識中心資料提出問題。

[AI聊天](ai-assistant.md)
:::

::::
